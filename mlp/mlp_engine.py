...
import numpy as np
import struct
import tensorrt as trt  # required for the model creation
import pycuda.autoinit  # required for the inference using TRT engine
...
def load_weights(file_path):
    ...
    # count for total 
    count = int (lines[0])
    assert count == len(lines) - 1, f"[ERROR]: {count} != {len(lines) - 1}"
    ...
    for i in range(1, count + 1):
        ...
        # len of splits must greater than current weight counts
...
def api_to_model(max_batch_size):
    ...
    # Create configuraions from Engine Builder
...
def perform_inference(input_val):
    ...
    def do_inference(inf_context, inf_host_in, inf_host_out):
        ...
        # Inout and output bindings are required for inference
        ...
        # copy input form host to device in stream
    ...
    # copy input-array from CPU to Flattrn array in GPU
