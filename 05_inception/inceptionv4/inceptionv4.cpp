...
namespace trtx {
    InceptionV4::InceptionV4(const InceptionV4Params &params) 
        :mParams(params), mEngine(nullptr), mContext(nullptr) {

    }

    bool InceptionV4::serializeEngine() {
        weightMap = loadWeights(mParams.weightFilePath);
        ...
        std::ofstream trtFile(mParams.trtEnginePath, std::ios::binary);
        if(!trtFile){
    ...
    bool InceptionV4::buildEngine(IBuilder *builder, IBuilderConfig *config) {
        ...
        IScaleLayer* scale1 = normalizedLayer(network, *data);
    ...
    void InceptionV4::doInference(float* input, float* output, int batchSize) {
        ...
        CHECK(cudaMemcpyAsync(buffers[inputIndex], input, batchSize * 3 * mParams.inputH * mParams.inputW * sizeof(float), cudaMemcpyHostToDevice, stream));
    ...
    bool InceptionV4::deserializeCudaEngine() {
        ...
        if (mEngine == nullptr) {
            ...
            // open file
            std::ifstream f(mParams.trtEnginePath, std::ios::binary);
