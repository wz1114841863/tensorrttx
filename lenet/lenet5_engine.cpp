...
const std::string WEIGTS_FILE_PATH = "./lenet5.wts";
const std::string ENGINE_FILE_PATH = "./lenet5_cpp.engine";

// 从 .wts 文件中加载权重
// Load weights from files shared with TensorRT samples.
// TensorRT weight files have a simple space delimited format:
// [type] [size] <data x size in hex>
std::map<std::string, Weights> loadWeightsFromFile(const std::string file) {
...
ICudaEngine *createLenetEneine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig* config, DataType dt) {
    INetworkDefinition *network = builder->createNetworkV2(0U);

    auto weightMap = loadWeightsFromFile(WEIGTS_FILE_PATH);

    // Create input tensor of shape{1, 32, 32}with name INPUT_BLOB_NAME
    ITensor *data = network->addInput(INPUT_BLOB_NAME, dt, Dims3{1, INPUT_H, INPUT_W});
    assert(data);

    // Add Convolution layer with 6 outputs and a 5*5 filter
    IConvolutionLayer *conv1 = network->addConvolutionNd(*data, 6, DimsHW{5, 5}, weightMap["conv1.weight"], weightMap["conv1.bias"]);
    assert(conv1);
    conv1->setStrideNd(DimsHW{1, 1});
    conv1->setPaddingNd(DimsHW{0, 0});

    // Add activation layers using the RELU layer
...
void APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream) {
    ...
    auto engine = createLenetEneine(maxBatchSize, builder, config, DataType::kFLOAT);
    assert(engine != nullptr);

    // Serialzie the engine
