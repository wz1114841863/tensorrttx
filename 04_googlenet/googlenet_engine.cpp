...
IConcatenationLayer *inception(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, 
        ITensor &input, std::string lname, int ch1x1, int ch3x3red, int ch3x3, int ch5x5red, int ch5x5, int pool_proj) {
...
ICudaEngine *createEngine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt) {
    ...
    // Create input tensor of shape {1, 1, 32, 32} with name of INPUT_BLOB_NAME
    ITensor *data = network->addInput(INPUT_BLOB_NAME, dt, Dims3{3, INPUT_H, INPUT_W});
    assert(data);

    std::string wthPath = "../googlenet.wts";
    auto weightMap = loadWeights(wthPath);
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
...
void APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream, Logger &gLogger){
    // Create builder
...
int main(int argc, char** argv) {
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        APIToModel(1, &modelStream, gLogger);
        assert(modelStream != nullptr);

        std::ofstream p("./googlenet_cpp.engine", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("googlenet_cpp.engine", std::ios::binary);
        if (file.good()) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        doInference(*context, data, prob, 1, INPUT_BLOB_NAME, OUTPUT_BLOB_NAME, INPUT_H, INPUT_H, OUTPUT_SIZE);
