...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../mnasnet0_5.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};

    IConvolutionLayer* conv1 = network->addConvolutionNd(*data, 16, DimsHW{3, 3}, weightMap["layers.0.weight"], emptywts);
    assert(conv1);
    ...
    IConvolutionLayer* conv2 = network->addConvolutionNd(*relu1->getOutput(0), 16, DimsHW{3, 3}, weightMap["layers.3.weight"], emptywts);
    assert(conv2);
    conv2->setPaddingNd(DimsHW{1, 1});
    conv2->setNbGroups(16);

    IScaleLayer* bn2 = addBatchNorm2d(network, weightMap, *conv2->getOutput(0), "layers.4", 1e-5);
    IActivationLayer* relu2 = network->addActivation(*bn2->getOutput(0), ActivationType::kRELU);
    assert(relu2);

    IConvolutionLayer* conv3 = network->addConvolutionNd(*relu2->getOutput(0), 8, DimsHW{1, 1}, weightMap["layers.6.weight"], emptywts);
    assert(conv3);
    IScaleLayer* bn3 = addBatchNorm2d(network, weightMap, *conv3->getOutput(0), "layers.7", 1e-5);

    ILayer* ir1 = invertedRes(network, weightMap, *bn3->getOutput(0), "layers.8.0.", 8, 16, 3, 2, 3);
