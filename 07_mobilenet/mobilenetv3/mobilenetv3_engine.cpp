...
ILayer *hSwish(INetworkDefinition *network, ITensor &input, std::string name) {
    // 使用hard sigmoid 构造 hard swish，hardswish(x) = x * hardSigmoid(x)
...
ILayer *convBnHswish(INetworkDefinition *network, std::map<std::string, Weights> weightMap, ITensor &input, 
...
ILayer *seLayer(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, 
        ITensor &input, int c, int w, std::string lname) {
    int h = w;
    IPoolingLayer *l1 = network->addPoolingNd(input, PoolingType::kAVERAGE, DimsHW{w, h});
    assert(l1);
    l1->setStrideNd(DimsHW{w, h});

    IFullyConnectedLayer* l2 = network->addFullyConnected(*l1->getOutput(0), BS * c/4, weightMap[lname+"fc0.weight"], weightMap[lname+"fc0.bias"]);
    IActivationLayer* relu1 = network->addActivation(*l2->getOutput(0), ActivationType::kRELU);
    IFullyConnectedLayer* l4 = network->addFullyConnected(*relu1->getOutput(0), BS * c, weightMap[lname+"fc1.weight"], weightMap[lname+"fc1.bias"]);
...
ILayer *convSeq1(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, 
            ITensor &input, int output, int hdim, int k, int s, bool use_se, bool use_hs, int w, std::string lname) {
    Weights emptyWts{DataType::kFLOAT, nullptr, 0};
    int p = (k - 1) / 2;
    ...
    IScaleLayer *bn1 = addBatchNorm2d(network, weightMap, *conv1->getOutput(0), lname + "1", 1e-5);
    ITensor *tensor3 = nullptr, *tensor4 = nullptr;
    if (use_hs) {
        ILayer* hsw = hSwish(network, *bn1->getOutput(0), lname+"2");
    ...
    IScaleLayer* bn2 = addBatchNorm2d(network, weightMap, *conv2->getOutput(0), lname + "5", 1e-5);
    assert(bn2);
    return bn2;
}

ILayer* convSeq2(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, 
            ITensor& input, int output, int hdim, int k, int s, bool use_se, bool use_hs, int w, std::string lname) {
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    int p = (k - 1) / 2;
    ...
    IScaleLayer* bn3 = addBatchNorm2d(network, weightMap, *conv3->getOutput(0), lname + "8", 1e-5);
    assert(bn3);
    return bn3;
}

ILayer* invertedRes(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, 
        ITensor& input, std::string lname, int inch, int outch, int s, int hidden, int k, bool use_se, bool use_hs, int w) {
...
ICudaEngine* createEngineSmall(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../mobilenetv3.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    assert(fc1);
    ILayer* bn1 = addBatchNorm2d(network, weightMap, *fc1->getOutput(0), "classifier.1", 1e-5);
    ...
    ILayer* bn2 = addBatchNorm2d(network, weightMap, *fc2->getOutput(0), "classifier.4", 1e-5);
...
void APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream) {
    ...
    ICudaEngine* engine = createEngineSmall(maxBatchSize, builder, config, DataType::kFLOAT);
