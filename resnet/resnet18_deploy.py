...
def basicBlock(network, weight_map, input, output, layer_name):
    ...
    ew1 = network.add_elementwise(input, bn2.get_output(0), trt.ElementWiseOperation.SUM)
    assert ew1
...
def bottleNeck(network, weight_map, input, output, layer_name):
    conv1 = network.add_convolution(input=input,
                                    num_output_maps=output,
                                    kernel_shape=(3, 3),
                                    kernel=weight_map[layer_name + "conv1.weight"],
    ...
    conv3 = network.add_convolution(input=input, 
                                    num_output_maps=output,
                                    kernel_shape=(1, 1),
    ...
    bn3 = addBatchNorm2d(network, weight_map, conv3.get_output(0), layer_name + "downsample.1", EPS)
    assert bn3

    ew1 = network.add_elementwise(bn3.get_output(0), bn2.get_output(0),
...
def create_engine(maxBatchSize, builder, config, dt):
    ...
    x = basicBlock(network, weight_map, pool1.get_output(0), 64, "layer1.0.")
    x = basicBlock(network, weight_map, x.get_output(0), 64, "layer1.1.")
    x = bottleNeck(network, weight_map, x.get_output(0), 128, "layer2.0.")

    x = basicBlock(network, weight_map, x.get_output(0), 128, "layer2.1.")
    x = bottleNeck(network, weight_map, x.get_output(0), 256, "layer3.0.")

    x = basicBlock(network, weight_map, x.get_output(0), 256, "layer3.1.")
    x = bottleNeck(network, weight_map, x.get_output(0), 512, "layer4.0.")

    x = basicBlock(network, weight_map, x.get_output(0), 512, "layer4.1.")
