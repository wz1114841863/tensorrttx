...
IActivationLayer *basicBlock(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, 
    ...
    Weights emptywts{DataType::kFLOAT, nullptr, 0};  // bias weights
...
int main(int argc, char** argv) {
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("resnet18_cpp.engine", std::ios::binary);
        if (!p)
        {
        ...
        std::ifstream file("./resnet18_cpp.engine", std::ios::binary);
        if (file.good()) {
