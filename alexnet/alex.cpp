...
// struct wo konw about the network and the input/output blobs
...
// Create the engine using only the API and not parser.
...
void doInference(IExecutionContext &context, float *input, float *output, int batchSize) {
    ...
    // Pointers to input and output device buffers to pass to endine
    ...
    // DMA input batch data to device, infet on the batch asynchronously, and DMA output back to host
...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("alexnet_cpp.engine", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("alexnet_cpp.engine", std::ios::binary);
        if (file.good()) {
    ...
    // Print histogarm of the output distribution
    std::cout << "\nOutput:\n\n";
